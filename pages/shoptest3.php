<?php
    // shoptest2.php - The Dynamic Product Catalogue

    // 1. START SESSION (Essential for saving the cart later)
    if (session_status() === PHP_SESSION_NONE) {
        session_start();
    }
    
    // 2. INCLUDE DATABASE CONFIGURATION
    // This file must contain your connectDB() function
    require_once('../api/config.php');

    // Default variables (These prevent errors if the DB connection fails)
    $allProductsJson = '[]'; 
    $uniqueCategories = [];
    $uniqueBrands = [];
    $minPrice = 0;
    $maxPrice = 10000; // Safe initial range
    
    try {
        $pdo = connectDB();
        
        // A. FETCH INITIAL PRODUCTS (The JavaScript will use this array)
        $stmt = $pdo->prepare('SELECT 
            id, name, brand, category, price, item_condition, image, stock_quantity 
            FROM products
            LIMIT 30'); // Fetch a small batch for initial load

        $stmt->execute();
        $products = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $productCount = count($products);
        echo "$productCount products loaded.";
        
        // Convert the PHP array into a JSON string for JavaScript
        $allProductsJson = json_encode($products);
        
        // B. FETCH FILTER VALUES (For dynamic sidebar generation)
        ...
        // C. FETCH MIN/MAX PRICES (For price slider boundaries)
        ...
        if ($priceRange && $priceRange['min_price'] !== null) {
            $minPrice = floor(floatval($priceRange['min_price']));
            $maxPrice = ceil(floatval($priceRange['max_price']));
        }

    } catch (\Exception $e) {
        // Log the error but the page continues with empty data ($allProductsJson = '[]')
        error_log("Database Error in shop.php: " . $e->getMessage());
        // For now, let's echo a comment so you can see if the DB failed in your source code:
        echo "";
    }
?>
